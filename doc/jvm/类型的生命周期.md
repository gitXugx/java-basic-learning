...
可以编译代码会生成对应的class文件，这时删除User.class,运行程序结果如下:
```text
...
对于这两块，java编译器会把他们编译到一个<static>方法中(在深入java虚拟机第二版中说的是<clinit>,但是从jdk8看到的是<static>)，由虚拟机执行,static方法中静态变量是顺序至上而下的
```text
...
```
静态字段编译后会放在静态代码块的后面，导致此时读取的并不是最终的值。
